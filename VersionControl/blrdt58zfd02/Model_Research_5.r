...
sink(file="Model/Model_Research_5.txt")
startTime <- Sys.time()
...
save(clf, file="Model_Research_5.RData")
...
write.csv(submission, "Model/Model_Research_5.csv", row.names=FALSE)

# varImp <- xgb.importance(feature_names=feature.names, model=clf)
# write.csv(varImp, "Model/varImp_Model_Research_5.csv", row.names=FALSE)
endTime <- Sys.time()
