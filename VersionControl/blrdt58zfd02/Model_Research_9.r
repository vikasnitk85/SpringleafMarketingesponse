...
sink(file="Model/Model_Research_9.txt")
startTime <- Sys.time()
...
write.csv(submission, "Model/Model_Research_9.csv", row.names=FALSE)

varImp <- xgb.importance(feature_names=feature.names, model=clf)
write.csv(varImp, "Model/varImp_Model_Research_9.csv", row.names=FALSE)
endTime <- Sys.time()
