...
sink(file="Model/Model_Research_7.txt")
startTime <- Sys.time()
...
clf <- xgb.train(params              = param, 
                 ...
                 watchlist           = watchlist,
                 maximize            = TRUE)
save(clf, file="Model/Model_Research_7.RData")
#----------------------------------------------------------------------------
...
write.csv(submission, "Model/Model_Research_7.csv", row.names=FALSE)

# varImp <- xgb.importance(feature_names=feature.names, model=clf)
# write.csv(varImp, "Model/varImp_Model_Research_7.csv", row.names=FALSE)
endTime <- Sys.time()
