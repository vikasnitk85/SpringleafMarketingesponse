...
sink(file="Model/Model_Research_21.txt")
...
for(tmpVar in charVars) {
  ...
  dtrain[, tmpVar] <- as.integer(factor(dtrain[, tmpVar], levels=Levels))
  dval[, tmpVar] <- as.integer(factor(dval[, tmpVar], levels=Levels))
  scoreData[, tmpVar] <- as.integer(factor(scoreData[, tmpVar], levels=Levels))
}
...
# Adjust dtrain
dtrain <- dtrain[, setdiff(names(dtrain), zeroSD)]
dval <- dval[, setdiff(names(dval), zeroSD)]
...
clf <- xgb.train(params              = param,
                 ...
                 watchlist           = watchlist,
                 maximize            = TRUE)
save(clf, file="Model/Model_Research_21.RData")
...
write.csv(submission, "Model/Model_Research_21.csv", row.names=FALSE)
