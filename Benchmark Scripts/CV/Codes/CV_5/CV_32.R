...
Min <- sapply(train, function(x) min(x, na.rm=TRUE))
modVars <- names(which(Min==-1 & (nUnique==3 | nUnique==8 | nUnique==9 | nUnique==43 | nUnique==86)))
modVars

temp_train <- train[, modVars]
temp_train[temp_train==-1] <- NA
temp_test <- test[, modVars]
temp_test[temp_test==-1] <- NA
train[, names(temp_train)] <- temp_train
test[, names(temp_test)] <- temp_test
