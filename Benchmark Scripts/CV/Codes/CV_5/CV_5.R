...
# Converting date variables to Year and Month
train_date <- sapply(train_date, function(x) strptime(x, "%d%B%y:%H:%M:%S"))
train_date <- do.call(cbind.data.frame, train_date)
train_Year <- sapply(train_date, function(x) as.numeric(format(x, "%Y")))
train_Year <- data.frame(train_Year)
train_Month <- sapply(train_date, function(x) as.numeric(format(x, "%m")))
train_Month <- data.frame(train_Month)
names(train_Month) <- paste0(names(train_Month), "_MTHS")
train[, names(train_date)] <- train_Year
train <- cbind(train, train_Month)

test_date <- sapply(test_date, function(x) strptime(x, "%d%B%y:%H:%M:%S"))
test_date <- do.call(cbind.data.frame, test_date)
test_Year <- sapply(test_date, function(x) as.numeric(format(x, "%Y")))
test_Year <- data.frame(test_Year)
test_Month <- sapply(test_date, function(x) as.numeric(format(x, "%m")))
test_Month <- data.frame(test_Month)
names(test_Month) <- paste0(names(test_Month), "_MTHS")
test[, names(test_date)] <- test_Year
test <- cbind(test, test_Month)
