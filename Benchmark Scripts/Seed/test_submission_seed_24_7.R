...
modVars <- names(which(Max==99 & nUnique >= 3 & nUnique <= 10))
modVars
...
# Creating sqrt Variables
for(i in names(train)) {
  train[, paste0(i, "_SQRT")] <- sqrt(train[, i])
  test[, paste0(i, "_SQRT")] <- sqrt(test[, i])
  if(sum(is.infinite(train[, paste0(i, "_SQRT")])) > 0) train[which(is.infinite(train[, paste0(i, "_SQRT")])), paste0(i, "_SQRT")] <- NA
  if(sum(is.infinite(test[, paste0(i, "_SQRT")])) > 0) test[which(is.infinite(test[, paste0(i, "_SQRT")])), paste0(i, "_SQRT")] <- NA
}
