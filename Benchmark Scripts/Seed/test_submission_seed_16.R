...
#
columnSum <- colSums(train==-1, na.rm=TRUE)
tmpData <- train[, sort(names(columnSum[columnSum!=0]))]
tempUnique <- sapply(tmpData, function(x) length(unique(na.omit(x))))

for(i in names(tempUnique[tempUnique == 3])) {
  tmp <- as.numeric(as.factor(c(train[, i], test[, i])))
  train[, i] <- head(tmp, nrow(train))
  test[, i] <- tail(tmp, nrow(test))
}
...
preds_out <- predict(model, xgtest, ntreelimit = 5000)
