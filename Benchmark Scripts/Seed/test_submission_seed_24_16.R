...
MinValues <- sapply(train, min, na.rm=TRUE)
for(f in names(train)) {
   minValue <- min(c(train[, f], test[, f]), na.rm=TRUE)
   if(minValue <= 0) {
     train[, paste0("LOG_", f)] <- log(train[, f] + abs(minValue) + 1)
     test[, paste0("LOG_", f)] <- log(test[, f] + abs(minValue) + 1)
     train[, paste0("SQRT_", f)] <- sqrt(train[, f] + abs(minValue))
     test[, paste0("SQRT_", f)] <- sqrt(test[, f] + abs(minValue))
   } else {
     train[, paste0("LOG_", f)] <- log(train[, f])
     test[, paste0("LOG_", f)] <- log(test[, f])
     train[, paste0("SQRT_", f)] <- sqrt(train[, f])
     test[, paste0("SQRT_", f)] <- sqrt(test[, f])
   }
}
gc()
...
model = xgb.train(
  nrounds = 10000   # increase for more results at home
  , params = param0
  , data = xgtrain
  # , early.stop.round = 5
  , watchlist = watchlist
